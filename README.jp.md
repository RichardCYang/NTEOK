...
- **多様なブロックタイプ**: 段落、見出し、リスト、チェックリスト、画像、コード、数式、ボード、コールアウト、トグル、YouTube埋め込みなど
...
管理者アカウントでログインしてください。 **デフォルトのパスワードは提供されておらず**、`.env`の`ADMIN_USERNAME` / `ADMIN_PASSWORD`で設定した値が使用されます。
- ユーザー名: `.env`の`ADMIN_USERNAME`
- パ스워드: `.env`의 `ADMIN_PASSWORD`

> セキュリティ上の推奨事項: 強力なパスワードを設定し、初回ログイン後すぐに変更（または再設定）してください。
