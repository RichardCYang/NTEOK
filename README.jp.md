...
- **多様なブロックタイプ**: 段落、見出し、リスト、チェックリスト、画像、コード、数式、ボード、コールアウト、トグル、タブ、ファイル、カレンダー、YouTube埋め込みなど
...
### ノート編集
- **ブロックタイプ**: 段落、見出し(H1-H6)、リスト(箇条書き/番号)、画像、引用、コードブロック、水平線、LaTeX数式、ボード、コールアウト、トグル、タブ、ファイル、カレンダー、YouTube埋め込みなど
...
スクリプトが以下の情報を対話形式で入力受け付けます:
- データベースホスト、ポート、ユーザー名、パスワード
- 管理者アカウントID/パスワード (未入力時は自動生成)
- HTTPS設定 (DuckDNS)
...
setupスクリプトで設定した管理者アカウントでログインしてください。

> セキュリティ上の推奨事項: 強力なパスワードを設定し、初回ログイン後、必要に応じて変更してください。
...
│       ├── tab-node.js              # タブビューブロック
│       ├── file-node.js             # ファイル添付ブロック
│       ├── calendar-node.js         # カレンダーブロック
...
### 2026-02-28 データ一貫性と安定性の修正

- **WebSocket-REST競合状況の解決** - 同時編集時のドキュメント破損防止
- **WebSocket接続切断復復強化** - 編集分の喪失防止およびサーバー永続保存保証
- **権限検証の強化** - 永久削除時の他ユーザーページ連鎖削除防止
- **E2EE協業保存** - エンドツーエンド暗号化環境でのリアルタイム協業内容DB永続保存
- **ブロックサイズ調整改善** - データベースブロック行列サイズの動的調整

### 2026-02-27 新規ブロックタイプおよび機能追加

- **タブビューブロック** - 複数タブで構成されたコンテンツ表示
- **ファイルブロック** - ファイル添付とダウンロード機能
- **カレンダーブロック** - カレンダー形式のデータ表示
- **データベースブロック** - 構造化データ管理 (行列形式)
- **ウェブページリンクブロック** - 外部リンク埋め込み表示
- **WebRTCベースのリアルタイム同期** - 暗号化ストレージでもYjs同期対応
- **.env設定の便利さ** - バッチファイル/シェルスクリプトで簡単な環境設定

### 2026-02-26 エンドツーエンド暗号化(E2EE)協業機能追加

- **ストレージ単位E2EE** - ストレージ全体をエンドツーエンド暗号化で保護
- **暗号化環境リアルタイム協業** - Yjsベースの同期で暗号化されたページのリアルタイム共同編集

### 2026-02-23~02-25 セキュリティとアクセス制御の強化

- **CWE-400 DoS脆弱性修正** - Yjsドキュメント肥大化によるOOM攻撃防止 (ドキュメント上限追加)
- **メモリリーク DoS修正** - isomorphic-dompurify v3.0.0アップグレード (dompurify v3.3.2オーバーライド)および定期的window再生成
- **Broken Access Control(IDOR)修正** - 協業者による他ユーザーファイル無断削除防止 (所有者検証追加)
- **WebSocketセッション権限検証** - ページ暗号化/公開切り替え後の既存接続強制終了
- **アップロード容量制限回避防止** - マルチスレッド環境での正確なクォータ管理 (Promise直列化)
- **WebSocket最大接続数管理** - DoS攻撃によるメモリ枯渇防止
