...
### 2026-02-28 데이터 일관성 및 안정성 수정

- **웹소켓-REST 경쟁 상황 해결** - 동시 편집 시 문서 깨짐 방지
- **웹소켓 연결 끊김 복구 강화** - 편집 분실 방지 및 서버 영속 저장 보장
- **권한 검증 강화** - 영구 삭제 시 타 사용자 페이지 연쇄 삭제 방지
- **E2EE 협업 저장** - 종단간 암호화 환경에서 실시간 협업 내용 DB 영속 저장
- **블록 크기 조절 개선** - 데이터베이스 블록 행렬 크기 동적 조절

### 2026-02-27 신규 블록 타입 및 기능 추가

- **탭 뷰 블록** - 여러 탭으로 구성된 콘텐츠 표시
- **파일 블록** - 파일 첨부 및 다운로드 기능
- **캘린더 블록** - 달력 형식의 데이터 표시
- **데이터베이스 블록** - 구조화된 데이터 관리 (행렬 형식)
- **웹페이지 링크 블록** - 외부 링크 임베드 표시
- **WebRTC 기반 실시간 동기화** - 암호화 저장소에서도 Yjs 동기화 지원
- **.env 설정 편의** - 배치파일/쉘스크립트로 간편한 환경 설정

### 2026-02-26 종단간 암호화(E2EE) 협업 기능 추가

- **저장소 단위 E2EE** - 저장소 전체를 종단간 암호화로 보호
- **암호화 환경 실시간 협업** - Yjs 기반 동기화로 암호화된 페이지 실시간 공동 편집
...
- **WebSocket 최대 연결 수 관리** - DoS 공격으로 인한 메모리 고갈 방지
