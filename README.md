...
- **다양한 블록 타입**: 문단, 제목, 리스트, 체크리스트, 이미지, 코드, 수식, 보드, 콜아웃, 토글, YouTube 임베드 등
...
관리자 계정으로 로그인하세요. **기본 비밀번호는 제공되지 않으며** `.env`의 `ADMIN_USERNAME` / `ADMIN_PASSWORD`로 설정한 값이 사용됩니다.
- 아이디: `.env`의 `ADMIN_USERNAME`
- 비밀번호: `.env`의 `ADMIN_PASSWORD`

> 보안 권장: 강력한 비밀번호를 설정하고, 초기 로그인 후 즉시 변경(또는 재설정)하세요.
