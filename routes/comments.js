...
const SHARED_COMMENT_CSRF_SECRET = (() => {
    if (process.env.NODE_ENV === 'production' && !process.env.SHARED_COMMENT_CSRF_SECRET && !process.env.SESSION_SECRET) throw new Error('[보안] 운영 환경에서는 SHARED_COMMENT_CSRF_SECRET (또는 SESSION_SECRET) 설정이 필수입니다.');
    return String(process.env.SHARED_COMMENT_CSRF_SECRET || process.env.SESSION_SECRET || crypto.randomBytes(32).toString('hex'));
})();
...
module.exports = (dependencies) => {
    ...
    router.get(["/shared", "/shared/:token"], rejectTokenInUrlPath, async (req, res) => {
        ...
        try {
            ...
            logError("GET /api/comments/shared", e);
    ...
    router.post(["/shared", "/shared/:token"], rejectTokenInUrlPath, sharedCommentWriteLimiter, requireSameOriginForAuth, async (req, res) => {
        ...
        try {
            ...
            if (!userId) {
                ...
                if (!sid || typeof tokenFromHeader !== 'string') return res.status(403).json({ error: "CSRF 토큰이 필요합니다." });
                const expected = signSharedCommentCsrf(tokenHash, sid);
                if (!safeEqualHex(expected, tokenFromHeader)) return res.status(403).json({ error: "CSRF 토큰이 유효하지 않습니다." });
            }
            ...
            logError("POST /api/comments/shared", e);
